...
final class PhotoLibraryService {
    ...
    var assetNilCount = 0   // work around for 'Retrived asset is nil' in iOS 11 when iCloud Photo Sync is turned ON
    ...
    fileprivate func putMediaToAlbum(_ assetsLibrary: ALAssetsLibrary, url: URL, album: String, completion: @escaping (_ error: String?)->Void) {
        ...
        assetsLibrary.asset(for: url, resultBlock: { (asset: ALAsset?) in
            ...
            guard let asset = asset else {
                if (self.assetNilCount < 40) {
                  self.assetNilCount += 1
                  usleep(2500)
                  self.putMediaToAlbum(assetsLibrary, url: url, album: album, completion: completion)
                  return
                }
